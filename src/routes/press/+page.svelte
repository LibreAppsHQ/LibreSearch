...
<main class="min-h-screen bg-[var(--app-background)] text-[var(--app-text)]">
		...
		<dl class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
			{#each facts as fact}
				<div class="rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface)] p-6">
					...
					<dd class="mt-2 text-[15px] leading-6 text-[var(--app-text)]">{fact.value}</dd>
				</div>
			{/each}
		</dl>
	</section>
	...
	<section class="mx-auto w-full max-w-[1100px] px-6 py-12 sm:py-20">
		<div class="grid gap-10 lg:grid-cols-[1fr_2fr] lg:gap-16">
			...
			<div class="rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface)] p-6">
				<p class="text-[15px] leading-7 text-[var(--app-secondary)]">{boilerplate}</p>
				<button
					type="button"
					onclick={copyBoilerplate}
					class="mt-5 inline-flex items-center gap-2 rounded-lg border border-[var(--app-border)] px-4 py-2 text-sm font-medium text-[var(--app-text)] transition hover:bg-[var(--app-hover)]"
				>
	...
	<section id="assets" class="mx-auto w-full max-w-[1100px] scroll-mt-24 px-6 py-12 sm:py-20">
		...
		<div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
			{#each assets as asset}
				<div
					class="flex flex-col overflow-hidden rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface)]"
				>
					<div class="flex h-28 items-center justify-center bg-[var(--app-hover)] p-6">
						<img src={asset.file} alt={asset.name} class="max-h-full max-w-full" />
					</div>
					<div class="flex flex-1 flex-col gap-1 p-4">
						<p class="text-sm font-semibold text-[var(--app-text)]">{asset.name}</p>
						<p class="text-xs text-[var(--app-muted)]">{asset.note}</p>
						<a
							href={asset.file}
							download
	...
	<section class="mx-auto w-full max-w-[1100px] px-6 py-16 sm:py-24">
		<div
			class="flex flex-col items-start justify-between gap-6 rounded-3xl border border-[var(--app-border)] bg-[var(--app-surface)] p-8 sm:flex-row sm:items-center sm:p-10"
		>
			<div>
