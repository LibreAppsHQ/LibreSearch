<script lang="ts">
	...
	const markStyles: Record<Mark, { icon: string; color: string; label: string }> = {
		...
		na: { icon: 'fa-minus', color: 'text-[var(--app-muted)]', label: 'Not applicable' }
	};
</script>
...
<main class="bg-[var(--app-background)] text-[var(--app-text)]">
	...
	<section class="mx-auto w-full max-w-[1100px] px-6 pt-12 pb-10 text-center sm:pt-20">
		...
		<p class="mx-auto mt-5 max-w-2xl text-base leading-7 text-[var(--app-muted)] sm:text-lg">
			...
			<span class="font-medium text-[var(--app-muted)]">–</span> only when we can back it up.
		</p>
	</section>
	...
	<section class="mx-auto w-full max-w-[1200px] px-4 pb-16 sm:px-6">
		<div
			class="overflow-x-auto rounded-2xl border border-[var(--app-border)] bg-[#171b25]/80 backdrop-blur-sm"
		>
			<table class="w-full min-w-[820px] border-collapse text-sm">
							...
							<th class="px-3 py-5 text-center">
								<img
									...
									class={`mx-auto rounded-xl object-contain ${engine.size ?? DEFAULT_LOGO_SIZE} ${
										engine.accent
											? 'ring-1 ring-[var(--app-accent)] ring-offset-4 ring-offset-[#171b25]'
											: ''
									}`}
									loading="lazy"
								/>
				...
				<tbody>
					{#each sections as section (section.title)}
						<tr class="border-t border-[var(--app-border)]/60 bg-[var(--app-background)]/40">
							<td
								colspan={engines.length + 1}
								class="sticky left-0 px-5 py-2.5 text-[11px] font-semibold tracking-wider text-[var(--app-muted)] uppercase"
							>
								{section.title}
							</td>
						</tr>
						{#each section.rows as row (row.label)}
							<tr class="border-t border-[var(--app-border)]/40">
								<td class="sticky left-0 z-10 bg-[#171b25] px-5 py-4">
									<p class="font-medium text-[var(--app-text)]">{row.label}</p>
									{#if row.note}
										<p class="mt-1 text-xs text-[var(--app-muted)]">{row.note}</p>
									{/if}
								</td>
		...
		>
			...
			<span class="inline-flex items-center gap-2">
				<i class="fa-solid fa-minus text-[var(--app-muted)]"></i> Not applicable
			</span>
		</div>
	</section>
	...
	<section class="mx-auto w-full max-w-[900px] px-6 pb-20">
		<div class="rounded-2xl border border-[var(--app-border)] bg-[#171b25]/60 p-7">
