...
<svelte:head>
	...
	<meta
		...
		content="LibreSearch is a private search engine that does not log queries, build profiles, or show ads."
	/>
...
<main class="relative overflow-hidden bg-(--app-background) text-(--app-text)">
	...
	<section class="relative mx-auto w-full max-w-[1100px] px-6 py-16 text-center sm:py-24">
		<h1 class="mx-auto max-w-3xl text-4xl font-bold tracking-tight text-white sm:text-5xl lg:text-7xl">
			Search the web.<br />
			<span class="text-white/80">Without being tracked.</span>
		</h1>

		<p class="mx-auto mt-6 max-w-xl text-lg leading-8 text-white/90">
			LibreSearch is a private search engine that returns results without logging queries,
			building user profiles, or serving advertising.
		</p>

		<div class="mt-10 flex flex-wrap items-center justify-center gap-4">
			<img src="/icon.svg" alt="Privacy illustration" class="h-48 w-48 sm:h-64 sm:w-64" />
		</div>
	</section>
		...
		<div class="mx-auto flex max-w-6xl flex-col items-center gap-12 md:flex-row md:justify-between">
			...
			<div class="max-w-xl text-center md:text-left">
				...
				<p class="mt-6 text-lg text-white/85">
					LibreSearch is built around a simple principle: search should not require tracking or identity.
					Results are generated from an independent index and delivered without storing personal data.
				</p>

				<p class="mt-4 text-lg text-white/85">
					There are no query logs, no user profiles, and no cross-site tracking. Searches are not tied
					to an identity.
				</p>
			...
			<img src="/icon2.svg" alt="Private browsing illustration" class="w-full max-w-lg md:max-w-xl" />
		</div>
	</section>
	...
	>
		<div class="mx-auto flex max-w-6xl flex-col items-center gap-12 md:flex-row md:justify-between">
			...
			<img src="/icon3.svg" alt="Online tracking illustration" class="w-full max-w-lg md:order-1 md:max-w-xl" />
			...
			<div class="max-w-xl text-center md:order-2 md:text-left">
				...
				<p class="mt-6 text-lg text-white/85">
					Most search engines associate queries with persistent identifiers and use that data for profiling or advertising.
					LibreSearch does not store or build user profiles.
				</p>
			...
			{#each [
				{
					...
					desc: 'Search queries are not stored or associated with an identity.'
				},
				{
					icon: 'fa-database',
					title: 'No profiles',
					color: 'text-blue-400',
					bg: 'bg-blue-500/15',
					desc: 'We do not build advertising or behavioral profiles from search activity.'
				},
				{
					icon: 'fa-lock',
					title: 'Local storage',
					...
					bg: 'bg-violet-500/15',
					desc: 'Settings and preferences remain in the browser and are not synced.'
				}
			] as pillar, i (i)}
				<div class="rounded-2xl border border-(--app-border) bg-[#171b25]/80 p-7 backdrop-blur-sm">
					<div class="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl {pillar.bg} {pillar.color}">
		...
		<div class="grid gap-16 lg:grid-cols-2 lg:gap-24">
			...
			<div>
				...
				<p class="text-sm leading-7 text-(--app-muted)">
					Queries are forwarded through LibreSearch so the destination index does not receive identifying information.
					Results are then returned to the user without storing request history.
				</p>
			</div>

			<div class="space-y-3">
				{#each [
					{ icon: 'fa-keyboard', label: 'You enter a query', color: 'text-(--app-accent)', bg: 'bg-(--app-accent)/10' },
					{ icon: 'fa-server', label: 'Request is proxied anonymously', color: 'text-blue-400', bg: 'bg-blue-500/10' },
					{ icon: 'fa-globe', label: 'Results are returned', color: 'text-emerald-400', bg: 'bg-emerald-500/10' },
					{ icon: 'fa-display', label: 'No data is stored', color: 'text-violet-400', bg: 'bg-violet-500/10' }
				] as step, i (i)}
					<div class="flex items-center gap-4 rounded-2xl border border-(--app-border) bg-[#171b25]/80 px-5 py-4">
	...
	<section class="mx-auto w-full max-w-[1100px] px-6 py-12 sm:py-20">
		...
		<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
			{#each [
				{ icon: 'fa-newspaper', title: 'News', desc: 'Top stories from across the web.' },
				{ icon: 'fa-play', title: 'Videos', desc: 'Search video content.' },
				{ icon: 'fa-image', title: 'Images', desc: 'Visual search results.' },
				{ icon: 'fa-bolt', title: 'Instant answers', desc: 'Quick factual summaries.' },
				{ icon: 'fa-clock-rotate-left', title: 'Search history', desc: 'Optional local history.' },
				{ icon: 'fa-palette', title: 'Themes', desc: 'Light and dark modes.' }
			] as f, i (i)}
	...
	<section class="mx-auto w-full max-w-[800px] px-6 py-16 sm:py-24">
		...
		<div class="space-y-3">
			{#each [
				{ q: 'Do you log searches?', a: 'No. Queries are not stored or tied to identity.' },
				{ q: 'Do I need an account?', a: 'No account is required.' },
				{ q: 'How is this funded?', a: 'The project is independently maintained.' },
				{ q: 'Where is data stored?', a: 'Settings remain in the browser locally.' },
				{ q: 'Where do results come from?', a: 'An independent index and proxy system.' }
			] as item, i (i)}
				<details class="group rounded-2xl border border-(--app-border) bg-[#171b25]/80 px-6 py-5">
					<summary class="flex cursor-pointer list-none items-center justify-between gap-4 text-base font-semibold">
						{item.q}
						<i class="fa-solid fa-chevron-down text-sm text-(--app-muted) transition group-open:rotate-180"></i>
					</summary>
