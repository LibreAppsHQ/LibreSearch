...
<main class="bg-[var(--app-background)] text-[var(--app-text)]">
	...
	<section class="mx-auto w-full max-w-[1100px] px-6 py-16 text-center sm:py-24">
		...
		<h1 class="mx-auto max-w-3xl text-4xl font-bold tracking-tight sm:text-5xl lg:text-6xl">
			Private search,<br />
			<span class="text-[var(--app-accent)]">one click away.</span>
		</h1>
	...
	<section class="mx-auto w-full max-w-[1100px] px-6 pb-16">
		<div class="grid gap-4 sm:grid-cols-3">
			{#each browsers as b}
				<div
					class="flex flex-col gap-4 rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface)] p-6"
				>
					<div class="flex items-center justify-between">
						<i class="fa-brands {b.icon} text-3xl text-[var(--app-accent)]"></i>
						...
						{:else}
							<span
								class="rounded-full bg-[var(--app-hover)] px-2 py-0.5 text-[10px] font-semibold tracking-wider text-[var(--app-muted)] uppercase"
								>Coming soon</span
							>
						{/if}
					</div>
					<div>
						<h2 class="text-base font-semibold text-[var(--app-text)]">{b.name}</h2>
						<p class="mt-2 text-sm leading-6 text-[var(--app-muted)]">{b.note}</p>
					</div>
					{#if b.url}
						<a
							href={b.url}
							...
							class="mt-auto inline-flex items-center justify-center gap-2 rounded-xl bg-[var(--app-accent)] px-4 py-2 text-sm font-semibold text-[#0d1019] transition-opacity hover:opacity-90"
						>
					...
					{:else}
						<button
							type="button"
							disabled
							class="mt-auto inline-flex cursor-not-allowed items-center justify-center gap-2 rounded-xl border border-[var(--app-border)] bg-[var(--app-hover)] px-4 py-2 text-sm font-semibold text-[var(--app-muted)]"
						>
							Not yet available
						</button>
					{/if}
	...
	<section class="mx-auto w-full max-w-[1100px] px-6 py-16 sm:py-20">
		...
		<div class="grid gap-6 sm:grid-cols-2">
			{#each features as f}
				<div
					class="rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface)] p-6 transition-colors hover:bg-[var(--app-hover)]"
				>
					<div
					...
					<p class="mt-2 text-sm leading-6 text-[var(--app-muted)]">{f.desc}</p>
				</div>
			{/each}
		</div>
	</section>
	...
	<section class="mx-auto w-full max-w-[1100px] px-6 py-16 text-center sm:py-20">
		...
		<p class="mx-auto mt-4 max-w-lg text-sm leading-6 text-[var(--app-muted)]">
		...
		<div class="mt-8 flex flex-wrap items-center justify-center gap-4">
			<a
				...
				target="_blank"
				rel="noopener noreferrer"
				class="inline-flex items-center gap-2 rounded-xl border border-[var(--app-border)] bg-[var(--app-surface)] px-5 py-3 text-sm font-semibold text-[var(--app-text)] transition-colors hover:bg-[var(--app-hover)]"
			>
