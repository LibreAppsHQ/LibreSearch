	...
	{#if featured}
		<section class="mx-auto w-full max-w-[1100px] px-6 py-12 sm:py-16">
			<a
				...
				class="group block overflow-hidden rounded-3xl border border-[var(--app-border)] bg-[var(--app-surface)] p-8 transition hover:border-[var(--app-accent)] sm:p-10"
			>
				<div class="flex flex-wrap items-center gap-3">
					...
					<span class="text-xs text-[var(--app-muted)]">Latest</span>
				</div>
				<h2
					class="mt-5 max-w-3xl text-2xl font-bold tracking-tight transition group-hover:text-[var(--app-accent)] sm:text-3xl"
				>
					{featured.title}
				</h2>
				<p class="mt-4 max-w-2xl text-base leading-7 text-[var(--app-secondary)]">
					{featured.description}
				</p>
				<div class="mt-6 flex items-center gap-3 text-sm text-[var(--app-muted)]">
	...
	<section class="mx-auto w-full max-w-[1100px] px-6 py-12 sm:py-16">
		...
		<div class="grid gap-6 sm:grid-cols-2">
			{#each rest as post}
				<a
					href="/blog/{post.slug}"
					class="group flex flex-col rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface)] p-6 transition hover:border-[var(--app-accent)]"
				>
					<span
					...
					<h3
						class="mt-4 text-lg font-semibold tracking-tight transition group-hover:text-[var(--app-accent)]"
					>
						{post.title}
					</h3>
					<p class="mt-3 flex-1 text-sm leading-6 text-[var(--app-secondary)]">
						{post.description}
					</p>
					<div class="mt-5 flex items-center gap-3 text-xs text-[var(--app-muted)]">
