...
<header class="sticky top-0 z-20 bg-[var(--app-background)]">
	<div class="mx-auto w-full max-w-[1400px] px-6">
		<div class="grid grid-cols-[1fr_auto_1fr] items-center py-5">
			...
			<a
				href="/blog"
				class="justify-self-center text-2xl font-bold tracking-tight text-[var(--app-text)]"
			>
				Blog
			</a>
...
<main class="min-h-screen bg-[var(--app-background)] text-[var(--app-text)]">
	<article class="mx-auto w-full max-w-[760px] px-6 py-12 sm:py-16">
		...
		<div class="mt-8 flex flex-wrap items-center gap-3">
			...
			<time class="text-sm text-[var(--app-muted)]" datetime={post.date}>
				{formatDate(post.date)}
			</time>
			<span class="text-sm text-[var(--app-muted)]">·</span>
			<span class="text-sm text-[var(--app-muted)]">{post.readingMinutes} min read</span>
		</div>

		<h1 class="mt-5 text-3xl font-bold tracking-tight sm:text-4xl">{post.title}</h1>
		<p class="mt-5 text-lg leading-8 text-[var(--app-secondary)]">{post.description}</p>

		<p class="mt-6 text-sm text-[var(--app-muted)]">By {post.author}</p>

		<div class="mt-10 border-t border-[var(--app-border)] pt-10">
			<div class="prose-app">
				{@html post.body}
			</div>
		</div>
		...
		<!-- Soft CTA -->
		<div
			class="mt-14 flex flex-col items-start gap-4 rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface)] p-6 sm:flex-row sm:items-center sm:justify-between"
		>
			<div>
			...
			<a
				href="/"
				class="inline-flex shrink-0 items-center gap-2 rounded-xl bg-[var(--app-accent)] px-5 py-2.5 text-sm font-semibold text-[#111111] transition hover:opacity-90"
			>
	...
	{#if more.length}
		<section class="mx-auto w-full max-w-[760px] px-6 pb-16">
			<div class="border-t border-[var(--app-border)] pt-10">
				<h2 class="mb-6 text-lg font-bold tracking-tight">Keep reading</h2>
				<div class="grid gap-4 sm:grid-cols-2">
					{#each more as p}
						<a
							href="/blog/{p.slug}"
							class="group rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface)] p-5 transition hover:border-[var(--app-accent)]"
						>
							<span
								class="inline-flex items-center rounded px-2 py-0.5 text-[10px] font-semibold tracking-wider uppercase {categoryMeta[
									p.category
